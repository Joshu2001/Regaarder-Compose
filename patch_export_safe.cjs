...
// We replace the compose dropdown logic manually by string indexing to be safe
const composeExportTarget = \`            <button
              onClick={() => {
                closeTransientMenus();
...
content = content.replace(composeExportTarget, newComposeExportButton);

// Replay Speed array and calc:
content = content.replace(
  /\}, Math\.max\(160, Math\.round\(650 \/ Math\.max\(0\.25, replaySpeed\)\)\)\);/g,
  "}, Math.max(30, Math.round(650 / Math.max(0.25, replaySpeed))));"
);

content = content.replace(
  /\{\[0\.25, 0\.5, 1, 1\.5, 2\]\.map\(\(speedOption\) => \(/g,
  "{[0.5, 1, 1.5, 2, 3, 4, 5].map((speedOption) => ("
);


fs.writeFileSync(appPath, content, 'utf8');
console.log('App.jsx patched carefully');
