...
// 1. Prepare modern room code with our screen share logic
const modernRoom = `
...
if (!content.includes('const renderRoomStage = () => {')) {
  // Inject just before the final return of App
  const appReturnIndex = content.indexOf('return (', content.lastIndexOf('return ('));
  if (appReturnIndex !== -1) {
    content = content.slice(0, appReturnIndex) + modernRoom + '\n  ' + content.slice(appReturnIndex);
  }
} else {
  // Replace existing renderRoomStage
  const startIndex = content.indexOf('const renderRoomStage = () => {');
  const endIndex = content.indexOf('};', startIndex) + 2;
  content = content.slice(0, startIndex) + modernRoom + content.slice(endIndex);
}

// Ensure the render call is actually inside the JSX tree.
const injectionTarget = "{/* Dynamic Toast System */}";
if (content.includes(injectionTarget) && !content.includes('{renderRoomStage()}')) {
  content = content.replace(injectionTarget, "{renderRoomStage()}\n      " + injectionTarget);
}

// Ensure we have PhoneOff icon imported
if (!content.includes('PhoneOff')) {
  content = content.replace('MicOff,', 'MicOff, PhoneOff,');
}

fs.writeFileSync(file, content, 'utf8');
console.log('Successfully injected premium Room UI');
