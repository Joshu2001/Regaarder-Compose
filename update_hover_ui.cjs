...
const oldMenuMatch = app.match(/\{hoveredBlockMenu && \(\s*<div\s*id="block-hover-menu"[\s\S]*?<\/div>\s*\)\}/);
...
if (oldMenuMatch) {
  ...
  app = app.replace(oldMenuMatch[0], newMenu);
  fs.writeFileSync('src/App.jsx', app);
  console.log('Hover UI improved.');
} else {
  console.log('Could not find old hover menu.');
}
