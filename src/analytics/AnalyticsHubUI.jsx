...
export default function AnalyticsHubUI({ activeSheetGrid, activeSheetId, updateSheetCell, showToast }) {
  ...
  return (
    <div className="w-full h-full flex flex-col bg-slate-50 dark:bg-zinc-950 text-slate-900 dark:text-zinc-100 overflow-y-auto thin-scrollbar p-6 space-y-6">
      ...
      {/* CLEAN PAGE HEADER                                    */}
      {/* ---------------------------------------------------- */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 pb-3 border-b border-slate-200/80 dark:border-zinc-800">
        <div>
          <div className="flex items-center gap-2">
            <span className="p-1.5 bg-gradient-to-br from-violet-600 to-indigo-700 text-white rounded-xl shadow-xs">
              <Zap size={18} />
            </span>
      ...
      <div className="bg-gradient-to-r from-violet-950 via-indigo-950 to-slate-900 border border-violet-800/30 text-white rounded-2xl p-3.5 sm:p-4 shadow-lg space-y-2">
        <form onSubmit={handleAiQuestionSubmit} className="flex items-center gap-2">
          ...
          {/* Data Range Selector (Integrated into search row) */}
          <div className="relative shrink-0" ref={selectDataRef}>
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2.5 bg-white/10 dark:bg-zinc-900/40 border border-white/15 hover:bg-white/15 rounded-xl text-xs font-semibold text-slate-200 transition-all cursor-pointer"
            >
              <Table size={13} className="text-violet-300" />
        ...
        {/* Default State: Quiet Secondary Plain Text Examples */}
        {!aiQuestionResult && (
          <div className="text-[11px] text-slate-400 font-normal flex flex-wrap items-center gap-1.5 pt-0.5">
            ...
            <button
              ...
              onPointerDown={(e) => {
                e.preventDefault();
                const q = "Why did churn increase?";
                setAiQuestionInput(q);
                const res = modules.routeBusinessQuestion(q, activeSheetGrid?.cells);
                setAiQuestionResult(res);
              }}
            ...
            <button
              ...
              onPointerDown={(e) => {
                e.preventDefault();
                const q = "Calculate break-even price";
                setAiQuestionInput(q);
                const res = modules.routeBusinessQuestion(q, activeSheetGrid?.cells);
                setAiQuestionResult(res);
              }}
      ...
      {/* ---------------------------------------------------- */}
      <div className="w-full shrink-0 bg-slate-200/80 dark:bg-zinc-800/80 p-1.5 rounded-2xl flex items-center gap-1.5 overflow-x-auto thin-scrollbar [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden border border-slate-200/80 dark:border-zinc-700/80 min-h-[48px]">
        {[
          ...
          return (
            <button
              ...
              className={`flex-1 min-w-[110px] h-9 px-4 rounded-xl text-xs font-bold transition-all cursor-pointer whitespace-nowrap flex items-center justify-center text-center shrink-0 leading-normal ${
                isSelected
                  ? 'bg-white dark:bg-zinc-900 text-violet-700 dark:text-violet-300 shadow-sm outline outline-2 outline-violet-600 dark:outline-violet-500 ring-2 ring-violet-500/20'
                  : 'text-slate-600 dark:text-zinc-400 hover:text-slate-900 dark:hover:text-zinc-200 hover:bg-slate-100/60 dark:hover:bg-zinc-800/60 border border-transparent'
              }`}
            >
              <span>{cat.label}</span>
            </button>
        ...
        {currentCategory.items.map((item) => {
          ...
          return (
            <button
              ...
              className={`p-3.5 text-left rounded-2xl transition-all cursor-pointer flex flex-col justify-between group min-h-[76px] ${
                isSelected
                  ? 'bg-violet-50/90 dark:bg-violet-950/60 border-2 border-violet-500 text-violet-950 dark:text-violet-100 shadow-sm ring-2 ring-violet-500/20'
                  : 'bg-white dark:bg-zinc-900 border border-slate-200/80 dark:border-zinc-800 hover:border-slate-300 dark:hover:border-zinc-700 hover:bg-slate-50/50 dark:hover:bg-zinc-850 text-slate-900 dark:text-zinc-100 shadow-2xs'
              }`}
            >
              <div className="space-y-1">
