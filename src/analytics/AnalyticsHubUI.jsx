...
export default function AnalyticsHubUI({ activeSheetGrid, activeSheetId, updateSheetCell, showToast }) {
  ...
  return (
    <div className="w-full h-full flex flex-col bg-slate-50 dark:bg-zinc-950 text-slate-900 dark:text-zinc-100 overflow-y-auto thin-scrollbar p-6 space-y-6">
      ...
      {/* ---------------------------------------------------- */}
      <div className="bg-slate-200/60 dark:bg-zinc-800/60 p-1 rounded-xl flex items-center gap-1 overflow-x-auto thin-scrollbar border border-slate-200/50 dark:border-zinc-700/50">
        {[
          ...
          return (
            <button
              ...
              onPointerDown={(e) => {
                ...
                setSelectedAnalysis(taxonomy.find(t => t.id === cat.id).items[0].id);
              }}
              className={`flex-1 min-w-[95px] text-center py-1.5 px-3 rounded-lg text-xs font-semibold transition-all cursor-pointer whitespace-nowrap ${
                isSelected
        ...
        {currentCategory.items.map((item) => {
          ...
          return (
            <button
              ...
              className={`p-3 text-left rounded-xl border transition-all cursor-pointer flex flex-col justify-between group shadow-none ${
                isSelected
                  ? 'bg-slate-100/90 dark:bg-zinc-800/80 border-slate-300 dark:border-zinc-700'
                  : 'bg-white dark:bg-zinc-900 border-slate-200/80 dark:border-zinc-800 hover:border-slate-300 dark:hover:border-zinc-700 hover:bg-slate-50/50 dark:hover:bg-zinc-850'
              }`}
            >
              <div>
                <div className="text-xs font-semibold text-slate-900 dark:text-zinc-100 flex items-center justify-between">
                  <span>{item.label}</span>
                  {isSelected && <Check size={13} className="text-slate-700 dark:text-zinc-300 shrink-0" />}
                </div>
                <p className="text-[11px] text-slate-500 dark:text-zinc-400 mt-1 line-clamp-1 group-hover:line-clamp-none transition-all">
                  {item.desc}
                </p>
              </div>
            </button>
