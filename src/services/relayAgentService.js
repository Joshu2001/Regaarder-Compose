...
export function classifyRelayIntent(prompt) {
  ...
  const isDocCreation = /(create|make|start|draft|write|generate)\s+(a\s+)?(new\s+)?(document|doc|proposal|brief|memo|notes|report)/i.test(text);
  const isTaskSchedule = /(add|create|schedule|set|assign)\s+(a\s+)?(new\s+)?(task|todo|initiative|action item|meeting|deadline|reminder)/i.test(text) || /\bdue\s+(today|tomorrow|next|on|by)\b/i.test(text);
  const isSheetUpdate = /(update|set|change|write|fill)\s+(the\s+)?(sheet|cell|row|column|cells)\s+([a-z]\d+|\d+)/i.test(text) || /(update|modify)\s+(spreadsheet|sheets)/i.test(text);
  const isCitationQuery = /(where is|where does it mention|find in docs|search docs for|cite where|what doc discusses|reference for|show me where)/i.test(text) || /\b(citation|citations|source reference)\b/i.test(text);
  ...
  if (typeof onCallAi === 'function') {
    try {
      const aiRaw = await onCallAi({
        userPrompt: trimmed,
        systemPrompt: RELAY_AGENT_SYSTEM_PROMPT,
        customModel,
        customProvider
      });
