...
export default function ScreenShareSourceModal({ isOpen, onClose, onSelectSource }) {
  ...
  // ─── Shared canvas card style (Crisp, Squarish & Architectural) ─────────────
  const cardClass = (id) =>
    `p-4.5 rounded-xl border transition-all cursor-pointer flex flex-col justify-between min-h-[114px] relative ${
      selectedSourceId === id
        ? 'border-slate-400 dark:border-zinc-500 bg-slate-50/70 dark:bg-zinc-800/60 ring-1 ring-slate-400/40 dark:ring-zinc-500/40'
        : 'border-slate-200/80 dark:border-zinc-800 bg-white dark:bg-zinc-900 hover:border-slate-300 dark:hover:border-zinc-700 hover:bg-slate-50/30'
    }`;
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-[700px] bg-white dark:bg-zinc-900 rounded-2xl border border-slate-200/90 dark:border-zinc-800 shadow-[0_24px_64px_rgba(0,0,0,0.18)] overflow-hidden flex flex-col animate-in zoom-in-95 duration-200"
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div className={`w-8.5 h-8.5 rounded-lg ${preset.iconBg} ${preset.iconColor} flex items-center justify-center`}>
                        <Icon size={18} strokeWidth={1.6} />
                      </div>
                      {isSelected && (
                        <div className="w-4.5 h-4.5 rounded-full bg-slate-900 dark:bg-white text-white dark:text-slate-900 flex items-center justify-center shadow-xs">
