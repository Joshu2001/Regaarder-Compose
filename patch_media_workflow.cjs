...
// 1. Rename SLASH_OPTIONS 'image' to 'media'
app = app.replace(
  /{ key: 'image', label: 'Image', desc: 'Insert an image' }/g,
  "{ key: 'media', label: 'Media', desc: 'Insert an image, video or file' }"
);
app = app.replace(
  /{ key: 'image', label: 'Image', desc: 'Insert an AI image' }/g,
  "{ key: 'media', label: 'Media', desc: 'Insert media, files, or AI generation' }"
);

// 2. Define the new Modals & replace MediaPicker
const mediaWorkflowCode = `
...
const mediaPickerRegex = /const MediaPicker = \(\{ isOpen, setOpen, anchorEl \}\) => \{[\s\S]*?(?=const EmojiGalleryPicker =)/;

if (app.match(mediaPickerRegex)) {
  app = app.replace(mediaPickerRegex, mediaWorkflowCode);
  fs.writeFileSync('src/App.jsx', app);
  console.log("Successfully replaced MediaPicker with Unified Workflow Modals.");
} else {
  console.error("Could not find MediaPicker block to replace.");
}
